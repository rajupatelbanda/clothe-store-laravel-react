...
const Navbar = () => {
  ...
  return (
    <nav className="navbar navbar-expand-lg sticky-top shadow-lg py-2 transition-all main-navbar">
      <div className="container">
        {/* Mobile View: Brand & Toggler */}
        <div className="d-flex align-items-center justify-content-between w-100 d-lg-none mb-2">
          <Link className="navbar-brand fw-black fs-3 d-flex align-items-center m-0" to="/">
            ...
            <span className="text-white letter-spacing-tight small-caps fs-5">{settings?.site_name || 'Gemini Store'}</span>
          </Link>
          <button className="navbar-toggler border-0 shadow-none custom-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav">
            <i className="bi bi-list-nested fs-2 text-danger"></i>
          </button>
        </div>
        ...
        <div className="collapse navbar-collapse" id="navbarNav">
          <ul className="navbar-nav me-auto ms-lg-5 mb-3 mb-lg-0 text-center text-lg-start">
            <li className="nav-item"><Link className="nav-link px-3 fw-bold text-white hover-text-dark transition-all" to="/">HOME</Link></li>
            <li className="nav-item"><Link className="nav-link px-3 fw-bold text-white hover-text-dark transition-all" to="/shop">SHOP</Link></li>
            {pages.map(page => (
              <li className="nav-item" key={page.id}><Link className="nav-link px-3 fw-bold text-white hover-text-dark transition-all text-uppercase" to={`/page/${page.slug}`}>{page.title}</Link></li>
            ))}
          </ul>
          
          <ul className="navbar-nav align-items-center flex-row justify-content-center gap-3">
            <li className="nav-item">
              <Link className="nav-link position-relative d-flex align-items-center justify-content-center bg-white rounded-circle shadow-sm hover-scale transition-all" to="/wishlist" title="Wishlist" style={{width: '40px', height: '40px'}}>
                <i className="bi bi-heart-fill fs-6 text-danger"></i>
                {wishlist.length > 0 && <span className="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-dark text-white border border-2 border-white shadow-sm" style={{fontSize:'9px', zIndex: 5}}>{wishlist.length}</span>}
              </Link>
            </li>
            <li className="nav-item cart-dropdown-wrapper">
              <Link className="nav-link position-relative d-flex align-items-center justify-content-center bg-white rounded-circle shadow-sm hover-scale transition-all" to="/cart" title="Bag" style={{width: '40px', height: '40px'}}>
                <i className="bi bi-bag-check-fill fs-6 text-danger"></i>
                {cart.length > 0 && <span className="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-dark text-white border border-2 border-white shadow-sm" style={{fontSize:'9px', zIndex: 5}}>{cart.length}</span>}
              </Link>
              ...
              <div className="cart-dropdown shadow-xl border-0 rounded-4 p-3 bg-white">
                ...
                {cart.length > 0 && (
                  <div className="mt-3 pt-2">
                    <div className="d-flex justify-content-between mb-3 fw-black small text-dark">
                      <span>SUBTOTAL</span>
            ...
            {user ? (
              <li className="nav-item dropdown">
                <a className="nav-link dropdown-toggle text-white fw-bold d-flex align-items-center bg-white bg-opacity-20 p-1 rounded-pill px-3 shadow-sm border border-white border-opacity-50" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown">
                  <img src={`https://ui-avatars.com/api/?name=${user.name}&background=FF5E78&color=fff`} alt="user" width="28" height="28" className="rounded-circle border border-2 border-white me-2" />
                  <span className="small">{user.name.split(' ')[0]}</span>
                </a>
                <ul className="dropdown-menu dropdown-menu-end shadow-xl border-0 rounded-4 p-2 mt-3 animate-up text-start" aria-labelledby="navbarDropdown">
              ...
              <div className="d-flex align-items-center gap-2">
                <li className="nav-item d-none d-sm-block"><Link className="nav-link text-white fw-bold px-2 transition-all hover-text-dark" to="/login">LOGIN</Link></li>
                <li className="nav-item"><Link className="btn btn-white bg-white text-danger rounded-pill px-3 py-1 fw-black shadow-sm transition-all hover-scale small" to="/register">JOIN US</Link></li>
              </div>
            )}
          </ul>
        </div>
      ...
      <style>{`
                .main-navbar {
                    background: linear-gradient(135deg, #ff4d6d 0%, #ff85a1 100%) !important;
                }
                ...
                .hover-text-dark:hover { color: #4B0600 !important; }
